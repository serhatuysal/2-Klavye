#include "cip_tank.h"
void setBarPercent_CipTanki(CipTank* self, uint16_t percent) {

    int16_t percentNegative = 100 - percent;
    int16_t wavePercent = ((percentNegative * 50) / 100) + 10;
...
CipTank  cipTankOlustur(lv_obj_t* parent, lv_coord_t  x, lv_coord_t y) {
    ...
    tboru.setBarPercent = setBarPercent_CipTanki;
    tboru.wavePercent = 60;
    /* motor.off = off;
     motor.on = on;
     motor.error = error;*/
    return tboru;
